...
/** The generator writes this line for a section no agent produced a claim for. */
const EMPTY = "No claims recorded for this section.";
...
export function SectionList({ sections }: { sections: ReportSection[] }) {
  return (
    <section aria-labelledby="sections-heading">
      ...
      {sections.map((s) => {
        const empty = s.body_markdown.includes(EMPTY);
        ...
        return (
          <details key={s.id} className="section" data-section-id={s.id}>
            <summary>
              ...
              {empty ? (
                <span className="chip chip-empty" title="No agent produced a claim for this section, so there was nothing to check.">
                  No findings
                </span>
              ) : (
