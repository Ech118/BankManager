...
|---|---|---|---|---|---|
| B1 | **Real mock MCP server** (`mcp_server.server.build_server`) | P1 has landed `backends.py`, fixture repositories and mock tool tests, but `build_server` and every tool are still `NotImplementedError` (checked after merging `origin/main`). P3 built and tested against its own test double (`tests/e2e/support/fake_mcp.py`). | P1 | [request](../requests/2026-09-19-p3-to-p1-mock-mcp-server.md) | Point the P3 MCP/coordinator tests at the real server object; they pass unchanged. |
| B9 | **The repo is on two MCP SDK majors** (P1's `mcp>=1.2` resolves to 2.x and its tests use `mcp.Client`; P3 was on 1.x) | P3's client and test double now support **both** (137 tests pass on mcp 1.30 and 2.2, each in its own venv). The team should still pick one and pin it. Also: under 2.x tool errors are masked unless tools raise `ToolError`. | P1 / coordinator | [request](../requests/2026-09-19-mcp-sdk-major-version.md) | `mcp_server/requirements.txt` and `agents/requirements.txt` agree; the out-of-scope reason reaches the UI through the real server. |
